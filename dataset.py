...
# Function to create or update the count dictionary
def update_count(directory):
    count = {chr(i).lower(): len(os.listdir(f'{directory}/{chr(i)}')) for i in range(65, 91)}
    count['blank'] = len(os.listdir(f'{directory}/blank'))
    return count

# Function to save frame to the specified directory
def save_frame(key, frame, count, directory):
    key = key.upper()
    save_path = os.path.join(directory, key, str(count[key.lower()])) + '.jpg'
    cv2.imwrite(save_path, frame)

cap = cv2.VideoCapture(0)

count = update_count(directory)

while True:
    _, frame = cap.read()
    frame = cv2.flip(frame, 1)  # Flip the frame horizontally
    row, col, _ = frame.shape
    cv2.rectangle(frame, (col - 300, 40), (col, 300), (255, 255, 255), 2)  # Shifted to the right side
    cv2.imshow("Data", frame)
    roi = frame[40:300, col - 300:col]  # Corresponding slice for the ROI on the right
    cv2.imshow("ROI", roi)
    gray_frame = cv2.cvtColor(roi, cv2.COLOR_BGR2GRAY)
    resized_frame = cv2.resize(gray_frame, (48, 48))
    
    interrupt = cv2.waitKey(10) & 0xFF
    if interrupt in range(ord('a'), ord('z') + 1) or interrupt == ord('.'):
        key = chr(interrupt)
        if key == '.':
            key = 'blank'
        save_frame(key, resized_frame, count, directory)
        count = update_count(directory)
